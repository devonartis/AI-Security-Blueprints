...
audience: "Security architects, platform engineers, identity teams"
companion_to: "v1.4 (Executive Edition)"
supersedes: "v1.3 (Technical Edition)"
---
...
**Version:** 1.4 (Technical Edition, companion to v1.4 Executive Edition)
...
- [Component Details](#component-details)
  - [Component 1: Ephemeral Identity Issuance](#component-1-ephemeral-identity-issuance)
  - [Component 2: Short-Lived Task-Scoped Tokens](#component-2-short-lived-task-scoped-tokens)
  - [Component 3: Zero-Trust Enforcement](#component-3-zero-trust-enforcement)
  - [Component 4: Automatic Expiration and Revocation](#component-4-automatic-expiration-and-revocation)
  - [Component 5: Immutable Audit Logging](#component-5-immutable-audit-logging)
  - [Component 6: Agent-to-Agent Mutual Authentication](#component-6-agent-to-agent-mutual-authentication)
  - [Component 7: Delegation Chain Verification](#component-7-delegation-chain-verification)
  - [Component 8: Operational Observability](#component-8-operational-observability)
- [Case Study: CVE-2025-68664 (LangGrinch)](#case-study-cve-2025-68664-langgrinch)
- [Benefits](#benefits)
- [Trade-Offs and Limitations](#trade-offs-and-limitations)
- [Failure Modes and Graceful Degradation](#failure-modes-and-graceful-degradation)
- [Migration, Not Remediation](#migration-not-remediation)
- [Implementation Considerations](#implementation-considerations)
- [When to Use This Pattern](#when-to-use-this-pattern)
- [Known Uses and Production Validation](#known-uses-and-production-validation)
...
Visual diagrams are provided as separate pages for clean viewing. Each diagram includes context, color legends, and a link back to the relevant pattern section. Mermaid `.mmd` source files are co-located in [`diagrams/`](../diagrams/) for regeneration.

| Diagram | Description |
|---------|-------------|
| [System Architecture](../diagrams/01-system-architecture.md) | All 8 components and their relationships |
| [Credential Lifecycle](../diagrams/02-credential-lifecycle.md) | End-to-end flow: Spawn, Attest, Issue, Operate, Expire |
| [Zero-Trust Validation](../diagrams/03-zero-trust-validation.md) | Complete validation pipeline at resource servers |
| [Delegation Chain](../diagrams/04-delegation-chain.md) | Multi-agent delegation with scope attenuation |
| [Attest & Proxy Model](../diagrams/05-attest-and-proxy.md) | v1.4 issuance model with sidecar and Central Authority |
| [Threat Model](../diagrams/07-threat-model.md) | Defended threats vs. complementary controls needed |
...
7. **Privacy by Design:** Audit systems must sanitize sensitive data (PII, prompts, model outputs) while preserving forensic utility. Log metadata and content hashes, never raw content. *(NEW IN v1.3)*
8. **Secure Credential Injection:** Long-lived static credentials must never be passed via environment variables. Short-lived single-use bootstrap material may be delivered via environment variables when the credential service enforces a TTL of 30 seconds or less, single-use consumption, and a scope ceiling, and registration requires cryptographic proof of key possession. The threat the prohibition addresses is exfiltration of credentials that remain valid long after exfiltration; bootstrap material whose validity ends within seconds and one use does not present that threat. *(REVISED IN v1.4 — see AGENTWRIT reference design under Known Uses)*

### Eight Core Components

> **See also:** [System Architecture Diagram](../diagrams/01-system-architecture.md) for a visual overview of how all components interconnect.
...
| **8. Operational Observability** *(NEW v1.3)* | Standardized errors, KPIs, liveness monitoring, and denial tracing | Operational excellence and debuggability at scale |

---

## Threat Model

> **See also:** [Threat Model Diagram](../diagrams/07-threat-model.md) for a visual mapping of defended threats and complementary controls.
...
> **See also:** [Credential Lifecycle Diagram](../diagrams/02-credential-lifecycle.md) and [Attest & Proxy Model Diagram](../diagrams/05-attest-and-proxy.md)
...
#### The "Attest & Proxy" Issuance Model *(NEW IN v1.3)*

To prevent private key sprawl across agent instances, the recommended issuance model separates attestation from credential signing:

1. **Agent Sidecar acts as Attestor, not Issuer:** The sidecar collects attestation evidence and proxies it to the Central Authority. It does not hold signing keys.
2. **Key generation is local:** The agent sidecar generates a temporary key pair in memory. The private key never leaves the sidecar.
3. **Central Authority signs:** The Central Authority validates the attestation evidence and signs the agent's public key, issuing the credential synchronously.
4. **Bootstrap via Time-Bounded Single-Use Token:** The orchestrator delivers a short-lived bootstrap token to the agent sidecar. Acceptable delivery mechanisms include platform-native secure injection (Kubernetes Projected Service Accounts, AWS Nitro Enclaves, tmpfs volumes) and environment variables, provided the token enforces a TTL of 30 seconds or less, single-use consumption, and a scope ceiling, and registration requires the agent to prove possession of a locally generated keypair via signed nonce. The locally generated private key never leaves the agent sidecar.

This model ensures that only the Central Authority holds signing keys, reducing the attack surface compared to distributed key management.

#### Session Resumption and Crash Recovery *(NEW IN v1.3)*

When an agent process crashes and restarts within the same orchestration task, the system must handle credential continuity securely:

- **Re-attestation required:** The restarted agent must go through the full attestation flow again. It cannot reuse the previous agent's credentials.
- **New instance ID:** The restarted agent receives a new unique instance ID to distinguish its audit trail from the crashed instance.
- **Implicit revocation:** The previous agent's credentials are implicitly revoked when re-attestation completes.
- **Audit continuity:** The audit log links the crashed instance and restarted instance via the shared task ID and orchestration ID.

#### One-Time Launch Token Bootstrap *(NEW IN v1.3)*

For small deployments or environments without full platform attestation, a formalized One-Time Launch Token provides a practical bootstrap mechanism:

- The orchestrator generates a cryptographically random launch token with a TTL of 30 seconds and single-use enforcement, scoped to a ceiling that the agent's eventual credentials cannot exceed.
- The launch token is delivered to the agent. Acceptable mechanisms include environment variables, platform-native secure injection (Kubernetes Projected Service Accounts, tmpfs volumes), and hardware-attested channels (AWS Nitro Enclaves). Each channel has a different accidental-disclosure surface — environment variables are inherited by subprocesses and can be dumped by debugging tools, while file-based tokens require an explicit read. For short-lived single-use bootstrap material, the time and use bounds shrink the exploitation window faster than realistic accidental-disclosure paths can hit it, which is what makes environment-variable delivery acceptable for this credential shape and not for long-lived credentials.
- The agent generates a keypair locally; the private key never leaves the agent. The agent presents the launch token, its public key, and a signature over a server-issued nonce to the credential service. The launch token alone is not sufficient to register; key possession is proven at use.
- The credential service validates the launch token, verifies the signature against the presented public key, issues ephemeral credentials bound to that public key, and immediately invalidates the launch token.
- Any attempt to reuse the launch token is rejected and logged as a security event. Failed registrations on a single-use token are an observable signal to the orchestrator.

This approach trades the strength of platform attestation for operational simplicity while maintaining the core pattern invariant: no long-lived secrets, and possession of the launch token alone is never sufficient to obtain credentials. AGENTWRIT is the reference implementation of this approach.
...
#### Token Renewal for Long-Running Agents *(NEW IN v1.3)*

Some agent tasks legitimately exceed the default token TTL (e.g., multi-step analysis, extended data processing). Rather than issuing long-lived tokens, the pattern supports token renewal:

- **Same identity, new token:** The agent presents its current valid token and receives a new token with a fresh TTL. The agent's SPIFFE identity remains the same.
- **Scope cannot expand:** Renewed tokens carry the same or narrower scope as the original token. Renewal is not an opportunity to request additional permissions.
- **Renewal window:** Tokens should be renewed before expiration, not after. A recommended approach is to renew at 75% of TTL (e.g., at 3:45 for a 5-minute token).
- **Audit continuity:** Each renewal is logged as a distinct event, maintaining a complete credential lifecycle trail.
- **Renewal limit:** Implementations should set a maximum number of renewals per session to prevent indefinite credential extension.

#### Semantic Scopes *(NEW IN v1.3)*

For complex authorization scenarios, simple string-based scopes (e.g., `read:Customers:12345`) may be insufficient. The pattern supports structured, JSON-based scope claims to enable attribute-based access control (ABAC):
...
Semantic scopes allow resource servers to make fine-grained authorization decisions based on multiple attributes beyond simple resource:action patterns. When used, scope attenuation in delegation chains must apply to all constraint fields—not just the resource and action.

#### Token Bloat Management *(NEW IN v1.3)*

As scope claims, delegation chains, and semantic constraints grow, JWT tokens can exceed practical size limits for HTTP headers and network transmission. The pattern addresses this through a by-reference token mechanism:

- **Threshold:** When the token size (header + payload + signature) exceeds 4KB, the system should switch to by-reference tokens.
- **Mechanism:** Issue an opaque artifact handle in the JWT `jti` claim. Store the full token payload in a short-lived, high-speed cache (Redis/Memcached) with the same TTL as the token.
- **Validation:** Resource servers resolve the artifact handle to retrieve the full token payload during validation.
- **Tradeoff:** By-reference tokens require online validation (cache lookup) rather than offline JWT verification. This is acceptable for most deployments but may not suit fully disconnected scenarios.

---

### Component 3: Zero-Trust Enforcement

> **See also:** [Zero-Trust Validation Diagram](../diagrams/03-zero-trust-validation.md) for the complete validation pipeline.
...
#### Clock Skew Tolerance *(NEW IN v1.3)*

In distributed systems, clock drift between components is inevitable. Validators must accept a configurable leeway (recommended: 30 seconds) when checking `nbf` (not before) and `exp` (expiration) claims. Without this tolerance, legitimate tokens may be rejected due to minor clock differences between the issuing service and the validating resource server. However, excessive leeway undermines the security guarantees of short-lived tokens—30 seconds represents a reasonable balance for most environments.
...
#### High-Assurance Introspection *(NEW IN v1.3)*

Not all agent operations carry the same risk. For agents accessing high-sensitivity resources (PII, payment systems, healthcare data), the standard cached revocation check may be insufficient. The pattern defines a tiered validation model:

- **Standard validation:** JWT signature verification + cached revocation list check. Suitable for most operations. Achieves <10ms latency.
- **High-assurance validation:** Online introspection (synchronous check against the Central Authority) for every request. Local caching of revocation status is forbidden. Required for operations classified as "critical" by organizational policy.

Organizations should define a sensitivity classification for their resources and map each classification to the appropriate validation tier. The performance cost of high-assurance introspection (additional network round-trip) is the intentional tradeoff for stronger security guarantees on critical operations.
...
- Tamper-evident (cryptographic hash chaining) *(NEW IN v1.3)*
- Retained per compliance requirements

#### Tamper-Evident Hash Chaining *(NEW IN v1.3)*

To provide cryptographic proof that audit logs have not been modified after the fact, each log entry should include:

- `prev_hash`: SHA-256 hash of the previous audit event
- `event_hash`: SHA-256 hash of the current event concatenated with `prev_hash`

This creates an append-only chain where any modification to a historical entry invalidates all subsequent hashes, making tampering detectable. Validators can verify chain integrity by recomputing hashes from any starting point.

#### Data Sanitization *(NEW IN v1.3)*

Audit logs must balance forensic utility with privacy requirements. The pattern requires aggressive sanitization of sensitive data:

- **Never log:** Raw prompt inputs, model outputs, PII (names, emails, SSNs), or authentication secrets
- **Always log:** Metadata (token counts, content hashes, request sizes), agent identifiers, resource identifiers, timestamps, and outcomes
- **Hash sensitive identifiers:** When customer or user identifiers must be logged for correlation, use consistent hashing to enable forensic linking without exposing raw values

This principle ensures that a compromised or leaked audit log does not itself become a data breach.

#### Log Aggregation for High-Volume Operations *(NEW IN v1.3)*

In high-throughput scenarios (agents making hundreds of read requests per minute), logging every individual access event can overwhelm the audit system. The pattern permits aggregation for repetitive successful reads:

- **Aggregated format:** "Agent X accessed Resource Y: 500 times between T1 and T2"
- **Exception:** All failures, write operations, and delegation events must be logged individually and atomically—never aggregated
- **Tradeoff:** Aggregation reduces storage and processing costs but sacrifices per-request granularity for successful reads
...
#### Secure Discovery Binding *(NEW IN v1.3)*

For mutual authentication to be effective, agents must be able to discover each other's identities reliably. The pattern recommends binding agent SPIFFE IDs to the service discovery layer:

- Store the agent's SPIFFE ID hash in DNS TXT records or service registry metadata (Consul, Kubernetes service annotations)
- When an agent resolves another agent's address, it also retrieves the expected SPIFFE ID
- During the mutual authentication handshake, the presented SPIFFE ID is validated against the discovery-bound identity

This prevents man-in-the-middle attacks where an attacker redirects agent traffic to a malicious endpoint that presents a valid but wrong identity.

#### Heartbeat and Liveness Monitoring *(NEW IN v1.3)*

To detect agents that have become unresponsive (crashed, hung, or compromised), the pattern recommends a heartbeat mechanism:

- Agents periodically signal liveness to the credential service (recommended interval: 30 seconds)
- If an agent misses a configurable number of heartbeats (recommended: 3 consecutive misses), the credential service flags the agent for investigation
- For high-security environments, missed heartbeats can trigger automatic credential revocation
- Heartbeat signals should include the agent's current task state (active, idle, completing) to support operational monitoring

Heartbeats serve a dual purpose: operational health monitoring and security signal. An agent that suddenly stops sending heartbeats may have been compromised.

---

### Component 7: Delegation Chain Verification

> **See also:** [Delegation Chain Diagram](../diagrams/04-delegation-chain.md) for a visual walkthrough of scope attenuation across agent hops.
...
Credential infrastructure must be observable. Without standardized error reporting, metrics, and denial tracing, operators cannot diagnose authentication failures, developers cannot debug integration issues, and security teams cannot distinguish configuration errors from actual attacks.

#### Standardized Error Contracts

When access is denied, the system must return structured, machine-parsable errors rather than opaque failure codes. This is critical for AI agents, which may hallucinate or retry indefinitely when they receive unstructured error responses.

The pattern recommends RFC 7807 (Problem Details for HTTP APIs) format:
...
```json
{
  ...
  "status": 403,
  "detail": "Token scope 'read:Customers:12345' does not include 'write:Customers:12345'",
  "instance": "/v1/customers/12345"
}
```

**Security consideration:** Error details should be informative enough for debugging but must not leak information that aids attackers. Never include internal system state, signing keys, or revocation list contents in error responses.

#### Key Performance Indicators (KPIs)

The credential system should emit metrics to the organization's observability platform:

| Metric | Type | Purpose |
|--------|------|---------|
| Token issuance duration | Histogram | Detect issuance service degradation |
| Validation decisions (allow/deny) | Counter | Track authorization patterns |
| Revocation cache hit ratio | Gauge | Monitor cache effectiveness |
| Clock skew detections | Counter | Identify infrastructure drift |
| Delegation chain depth | Histogram | Track delegation complexity |
| Anomaly revocations triggered | Counter | Monitor automated security responses |
| Heartbeat miss rate | Gauge | Track agent health |

These metrics enable proactive detection of systemic issues before they cause outages or security incidents.

#### "Why-Denied" Tracing

When access is denied, the audit log must record the specific reason for denial with enough detail for forensic analysis. Examples:

- "Token expired 45 seconds ago"
- "Scope 'read:Orders:*' does not match requested resource 'Customers:12345'"
- "Delegation hop 3 failed: scope escalation detected (write > read)"
- "Agent revoked at 10:02:15 due to anomaly detection"

**Critical distinction:** Why-denied details are logged to the audit trail for operators and security teams. They are NOT returned to the calling agent in full detail (see Standardized Error Contracts for what agents see).
...
✅ Operational observability with standardized errors, KPIs, and denial tracing *(NEW IN v1.3)*
✅ Privacy-preserving audit logs with data sanitization *(NEW IN v1.3)*
...
✅ **Operational debugging** through standardized errors and why-denied tracing *(NEW IN v1.3)*
...
## Migration, Not Remediation

> **See also:** v1.4 Executive Edition, Section 7 ("The Window Is Now") and Section 8 ("Governance and the Existing IAM Investment") for the business-language counterpart to the argument below.

The eight components in this pattern are not milestones on a remediation roadmap. They are design choices made at the point of first agent deployment. A phased rollout — ship agents on shared credentials now, add identity later, add scoping after that, add attestation when there is time — concedes that the credential shape is wrong at deployment and then invests in making the wrong shape auditable. That is remediation, not design.
...
For organizations already running production agents on shared credentials, the move is not to inventory agents. Agents do not exist outside the workflow that invokes them; every instance ends when its task ends. What persists — and what should be inventoried — is the workflow. The trigger, the orchestrator, the shared credential, the downstream integrations. Each of those is a discrete, governable object. The work is a workflow-level migration: take each workflow one at a time, rebind it to per-instance identity and task-scoped credentials, and cut over the shared credential that every future invocation would have ridden. Migration, not remediation.

### Day-One Design Checklist

Implementers deploying this pattern are making eight design decisions, not executing six phases. Each decision binds a component from the Component Details section to a concrete technology choice the engineering team must make before the first agent runs. The checklist below is ordered by dependency — earlier decisions gate later ones — not by sequence over calendar time.

**Identity substrate (Component 1).**
- Choose an identity issuer: SPIFFE/SPIRE, MCP CIMD, a cloud workload identity service, or custom PKI.
- Define what a per-instance identity contains: at minimum instance ID, task context, orchestration lineage, and parent identity for delegation.
- Choose a Secret Zero bootstrap mechanism: hardware attestation (TPM, HSM), cloud metadata attestation, one-time launch tokens, or attest-and-proxy via a sidecar.

**Credential issuance (Component 2).**
- Define scope taxonomy. The format must be expressive enough to capture least privilege (for example `action:resource:identifier`) without becoming unmanageable.
- Map each agent task type to its minimum scope. Version the mapping as policy and review it as tasks evolve.
- Set default TTL to task duration plus a small margin, not to operational convenience. Token renewal handles long-running workflows; TTL extension does not.
- Decide by-reference versus by-value tokens based on scope payload size. Crossing the 4 KB threshold usually forces by-reference with introspection.

**Validation (Component 3).**
- Enforce validation at every resource server, not at the perimeter. Validate signature, expiration, scope, revocation status, and delegation chain on every request.
- Set a validation latency budget (target below 50 ms) and measure against it continuously.
- Implement clock skew tolerance for signature and expiration checks from day one, not after the first failed incident.

**Expiration and revocation (Component 4).**
- Implement a revocation path that is independent of token expiration. Expired-by-revocation and expired-by-TTL are different signals and should be logged as such.
- For critical agents, require high-assurance introspection (real-time revocation check rather than cached).

**Audit (Component 5).**
- Choose append-only storage with tamper-evident hash chaining. Retrofitting tamper-evidence onto an existing log is significantly harder than choosing it at the start.
- Sanitize PII and prompt content before it lands in the authorization log; agent transcripts belong to a different retention tier than authorization events.
- Plan for high-volume reads separately from writes. Aggregation is a day-one concern for auditable systems at scale.

**Agent-to-agent (Component 6).**
- Default to mTLS with identity-bound certificates. Bind discovery to identity, not to hostname alone.
- Define heartbeat cadence and the revocation action on missed heartbeats before agents start running, not after the first orphaned credential.

**Delegation (Component 7).**
- Define the delegation token format: how authority is captured, how scope attenuates, how chains link cryptographically.
- Set a maximum delegation depth.
- Implement chain verification at resource servers, not only at the issuer.

**Observability (Component 8).**
- Adopt RFC 7807 (or equivalent) for error contracts so "why denied" is traceable end-to-end.
- Define the KPI set up front: issuance rate, validation latency, revocation latency, delegation depth, anomaly rate.
- Wire anomaly detection into automatic revocation so anomalies produce action, not only alerts.

Every item above is a decision to make before the first production agent runs. None has a reasonable "we'll add it later" posture that does not end in remediation.

### Workflow Migration Playbook

For organizations with existing production agents on shared credentials, the migration unit is the workflow, not the agent. An agent does not outlive its task; a workflow does. The following steps apply to one workflow at a time.

**1. Inventory the workflow, not the agents.** Identify the trigger (schedule, event, user action), the orchestrator, the shared credential the workflow currently uses, and every downstream integration it touches. Each of these is a governable object; the agent instances that ride on them are not.

**2. Bound the blast radius before migrating.** Compute the current effective permissions of the shared credential. This is almost always broader than anyone remembers. Document the permissions the workflow actually needs, not the permissions the shared credential grants. The delta is the over-privilege that ephemeral credentialing is about to eliminate.

**3. Stand up the issuer alongside the existing credential.** Deploy the identity issuer, credential issuance service, and validation path for this workflow without removing the shared credential. Both paths exist temporarily. This is parallel operation, not cutover.

**4. Rebind the workflow to per-instance identity.** Modify the orchestrator so each invocation requests a per-instance identity and a task-scoped credential from the issuer. Every agent instance in this workflow now has its own identity and its own credential, bounded by the task.

**5. Validate parity.** Run the workflow on the new credential path and confirm it produces identical downstream effects. Audit logs from the new path should attribute every action to a specific agent instance and task — something the shared-credential logs could not do.

**6. Cut over the shared credential.** Rotate, narrow, or revoke the shared credential. All future invocations of this workflow run on the ephemeral path. The shared credential either no longer exists or is scoped so narrowly that re-using it for new workflows requires an explicit governance decision.

**7. Repeat per workflow.** The migration is not a single cutover event. It is a workflow queue: highest-risk workflows first (financial, PII, cross-tenant), then the rest. Workflows sharing an orchestrator can often share migration effort; workflows that don't are independent and can proceed in parallel.

### Effort Estimates

Effort is bounded per workflow, not per phase. For a workflow with a single orchestrator and a single downstream integration, a parity-validated cutover typically requires two to four weeks of engineering effort after the issuer infrastructure exists. The first workflow absorbs the infrastructure cost — standing up the issuer, writing the validation path, instrumenting audit logging — and typically requires six to ten weeks. Subsequent workflows move faster because they reuse the infrastructure. Workflows involving multi-agent delegation or high-assurance introspection add two to four weeks each.

Effort scales with workflow count and workflow complexity, not with calendar phases. Organizations running dozens of agent workflows should plan for a small dedicated team working a migration queue rather than a cross-cutting program with sequential deliverables.

### Standards to Track

While migrating, monitor the standards bodies whose drafts will shape interoperability: IETF WIMSE for workload identity, the OAuth working group for token formats and introspection, the MCP specification for agent-to-tool interaction, NIST AI RMF and NIST IR 8596 for risk-management framing, and OWASP Agentic Top 10 for threat model updates. Adopt standardized mechanisms when they stabilize; do not block first deployment on them.
...
7. **Operational Observability** *(NEW IN v1.3)*
   - RFC 7807 error responses for all denial paths
   - Prometheus/OpenTelemetry metrics for KPIs
   - Why-denied tracing in audit logs
   - Heartbeat and liveness monitoring for agents
   - Dashboard or alerting integration for operational health
...
**OWASP Non-Human Identities (NHI) Top 10 (2025)** *(NEW IN v1.3)*
This resource specifically addresses risks for non-human identities including AI agents. Key entries aligned with this pattern: NHI4 (Insecure Authentication), NHI5 (Overprivileged NHI), NHI7 (Long-Lived Secrets), and NHI9 (NHI Reuse). The pattern directly mitigates all four through cryptographic identity verification, task-scoped tokens, minute-level TTLs, and unique per-instance identities.
...
**Contributors:** Devon Artis, AI Security Team
**License:** CC BY-SA 4.0
**Edition:** Technical Edition (companion to v1.4 Executive Edition)
**Supersedes:** v1.3 (Technical Edition)
...
| 1.3 | February 2026 | Added Component 8 (Operational Observability) with standardized error contracts (RFC 7807), KPI metrics, and why-denied tracing. Added architectural principles: Privacy by Design and Secure Credential Injection. Added to Component 1: Attest & Proxy issuance model, Session Resumption/Crash Recovery, One-Time Launch Token bootstrap. Added to Component 2: Token Renewal for long-running agents, Semantic Scopes (ABAC), Token Bloat Management (4KB threshold, by-reference tokens). Added to Component 3: Clock Skew Tolerance. Added to Component 4: High-Assurance Introspection for critical agents. Added to Component 5: Tamper-evident hash chaining, Data Sanitization (PII/prompt redaction), Log Aggregation for high-volume reads. Added to Component 6: Secure Discovery Binding, Heartbeat and Liveness Monitoring. Added OWASP NHI Top 10 (2025) to standards alignment. Aligned with Requirements v2.2 and MVP Requirements v1.0. |
| 1.4 (Technical Edition) | April 2026 | **Replaced the Adoption Path section (six-phase rollout with timeline guidance) with a Migration, Not Remediation section.** The new section reframes the pattern as a set of design choices at first agent deployment, not a sequence of remediation phases, and is internally consistent with v1.4 (Executive Edition). Added a Day-One Design Checklist mapping each of the eight components to concrete decisions implementers must make before first production run, a Workflow Migration Playbook for organizations already running agents on shared credentials (migration unit is the workflow, not the agent), and per-workflow effort estimates to replace the per-phase timeline. Removed the Adoption Roadmap diagram reference (`diagrams/06-adoption-roadmap.md`). Updated document frontmatter to declare edition "Technical Edition" and companion relationship to v1.4 (Executive Edition). No changes to the eight technical components, threat model, case study, benefits, trade-offs, failure modes, implementation considerations, or known uses. v1.3 is retained as a historical snapshot for comparison. |
