...
| [Attest & Proxy Model](../diagrams/05-attest-and-proxy.md) | v1.3 issuance model with sidecar and Central Authority |
| [Adoption Roadmap](../diagrams/06-adoption-roadmap.md) | 6-phase adoption path with timelines |
...
8. **Secure Credential Injection:** Bootstrap secrets and credentials must never be passed via environment variables or stored in agent-accessible memory. Use platform-native secure injection mechanisms. *(NEW IN v1.3)*
...
4. **Bootstrap via Secure Injection:** The orchestrator injects a short-lived bootstrap token into the agent sidecar via platform-native mechanisms (Kubernetes Projected Service Accounts, AWS Nitro Enclaves, tmpfs volumes). **Environment variables must NOT be used for bootstrap secrets.**
...
- The orchestrator generates a cryptographically random launch token with a TTL of 30 seconds and single-use enforcement.
- The launch token is injected into the agent via secure injection (not environment variables).
- The agent presents the launch token exactly once to the credential service.
- The credential service validates the token, issues ephemeral credentials, and immediately invalidates the launch token.
- Any attempt to reuse the launch token is rejected and logged as a security event.

This approach trades the strength of platform attestation for operational simplicity while maintaining the core pattern invariant: no long-lived secrets.
...
> **See also:** [Adoption Roadmap Diagram](../diagrams/06-adoption-roadmap.md) for a visual timeline of all 6 phases.
